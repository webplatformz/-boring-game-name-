...
// At the time of writing all sitting members resolve to a P18 image, and the
// bulk of them are official Parliamentary Services studio portraits.
//
// Licensing: the source files have individual Commons file-page terms. The
// current set includes attribution-only permissions as well as CC BY-SA. The
// author, designated credit, licence and source are recorded in CREDITS.md and
// cached in src/data/raw/commons-imageinfo.json. build-members.mjs publishes
// those records separately in src/data/portrait-credits.json for the app's
// central Photo Credits page; they are not mixed into the member/game records.
//
...
const normaliseAuthor = (author) =>
  author === 'Unknown author Unknown author' ? 'Unknown author' : author
  ...
  for (const g of groups) {
    ...
    for (const page of payload?.query?.pages ?? []) {
      ...
      out[page.title] = {
        ...
        author: normaliseAuthor(stripHtml(meta.Artist?.value) || 'Unknown author'),
...
function renderCredits(rows) {
  const body = rows
    .map((r) => {
      const licence = r.licenceUrl
        ? `[${r.licence}](${r.licenceUrl})`
        : `${r.licence} permission (see source file page)`
      const author = normaliseAuthor(r.author)
      const credit = r.attribution ? `${author} — credit: ${r.attribution}` : author
      return `| ${r.name} | [${r.title.replace(/^File:/, '')}](${r.descriptionUrl}) | ${credit} | ${licence} | Square-cropped, resized to ${SIZE} × ${SIZE}, converted to WebP |`
    })
    .join('\n')
...
${SIZE} × ${SIZE} pixels and converted to WebP by Bundeshaus Pack.
